...
const NewProductRegion = () => {
  ...
  const onFormSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(
        `${process.env.REACT_APP_SERVER_PATH}/api/v1/product_groups/`,
        payload
      );
      setOpen(true);
      setTimeout(() => {
        history.push("/administration/product-group");
      }, 1500);
    } catch (error) {
          ...
          <div className="attribute-box-1 table-margin box-shadow">
            <div className="attribute-box-1-heading row">
              <div className="col-3 attribute-back-link">
                <Link to={"/administration/product-group"}>
                  <i class="bi bi-arrow-left" component={Link}></i> Back
                </Link>
              </div>
              <h4 className="col-9">Add a new Product Group</h4>
              <br />
            </div>
            <div className="attribute-box-1-editBox row ">
              <p className="col-md-5 col-sm-12">Product Group</p>
              <br />
              <input
...
export default NewProductRegion;
