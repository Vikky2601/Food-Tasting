...
const EditTastingRegion = () => {
  ...
  const handleDelete = async () => {
    await axios.delete(
      `${process.env.REACT_APP_SERVER_PATH}/api/v1/regions/${id}`
    );
    setSnackDel(true);
    setTimeout(() => {
      history.push("/administration/tasting-region");
    }, 1500);
  };
    ...
    <form onSubmit={onFormSubmit}>
      ...
      >
        ...
        <Snackbar
          ...
          open={snackDel}
          autoHideDuration={1500}
          message="Tasting Region Deleted"
          onClose={handleToClose}
          action={
            <React.Fragment>
              <IconButton
          ...
          <div className="attribute-box-1 table-margin box-shadow">
            <div className="attribute-box-1-heading row">
              <div className="col-3 attribute-back-link">
                <Link to={"/administration/tasting-region"}>
                  <i class="bi bi-arrow-left" component={Link}></i> Back
                </Link>
              </div>
              <h4 className="col-9">Edit Tasting Region (#ID {id})</h4>
              <br />
            </div>

            <div className="attribute-box-1-editBox row ">
              <p className="col-md-5 col-sm-12">Region</p>
              <br />
              <input
...
export default EditTastingRegion;
