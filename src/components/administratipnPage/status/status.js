...
export default function Status() {
  ...
  useEffect(() => {
    axios
      .get(
        ...
        }/api/v1/statuses/?limit=5&offset=${parseInt(
  ...
  const handleChangePage = (event, newPage) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_SERVER_PATH}/api/v1/statuses/?limit=${
  ...
  const handleSearch = () => {
    axios
      .get(
        `${process.env.REACT_APP_SERVER_PATH}/api/v1/statuses/?search=${search}`
      )
      .then((res) => {
        setRows(res.data.results);
      });
        ...
        <>
          ...
          <div className="box-shadow clear-both table-margin">
            <TableContainer component={Paper}>
              <Table className="table-body-text" aria-label="simple table">
                      ...
                      <TableRow key={row.id}>
                        ...
                        <TableCell className="table-body-text" align="center">
                          <Tooltip title="Edit" placement="top-start">
                            <IconButton
                              component={Link}
                              to={`/administration/status/edit/${row.id}`}
                              aria-label="file"
