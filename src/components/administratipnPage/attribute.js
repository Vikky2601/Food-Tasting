...
export default function Attribute() {
  ...
  useEffect(() => {
    axios
      .get(
        ...
        }/api/v1/attributes/?limit=5&offset=${parseInt(
  ...
  const handleSearch = () => {
    axios
      .get(
        `${process.env.REACT_APP_SERVER_PATH}/api/v1/attributes/?search=${search}`
      )
      .then((res) => {
        setRows(res.data.results);
      });
  ...
  const handleChangePage = (event, newPage) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_SERVER_PATH}/api/v1/attributes/?limit=${
      ...
      <div class="container-fluid">
        <div className="search-container search-res-cont">
          <div className="taste-btn-container width-fill ">
            <Link
              to="/administration/attribute/new-attribute"
                      ...
                      <TableRow key={row.id}>
                        ...
                        <TableCell className="table-body-text" align="center">
                          <Tooltip title="Edit Entry" placement="top-start">
                            <IconButton
                              component={Link}
                              to={`/administration/attribute/edit/${row.id}`}
                              aria-label="file"
                          ...
                          <Tooltip title="Show Details" placement="top-start">
                            <IconButton
                              component={Link}
                              to={`/administration/attribute/view/${row.id}`}
                              aria-label="save"
                              className={classes.margin}
                              size="small"
                            >
