...
export default function TestType() {
  ...
  useEffect(() => {
    axios
      .get(
        ...
        }/api/v1/tasting_types/?limit=5&offset=${parseInt(
  ...
  const handleSearch = () => {
    axios
      .get(
        `${process.env.REACT_APP_SERVER_PATH}/api/v1/tasting_types/?search=${search}`
      )
      .then((res) => {
        setRows(res.data.results);
      });
  ...
  const handleChangePage = (event, newPage) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_SERVER_PATH}/api/v1/tasting_types/?limit=${
      ...
      <div class="container-fluid">
        <div className="search-container search-res-cont">
          <div className="taste-btn-container width-fill ">
            <Link
              to="/administration/test-type/new-testtype"
                  ...
                  {rows
                    ...
                    .map((row) => (
                      <TableRow key={row.id}>
                        ...
                        <TableCell className="table-body-text" align="center">
                          {row.description_long_text}
                        </TableCell>
                        <TableCell className="table-body-text" align="center">
                          <Tooltip title="Edit" placement="top-start">
                            <IconButton
                              component={Link}
                              to={`/administration/test-type/edit/${row.id}`}
                              aria-label="file"
                          ...
                          <Tooltip title="View" placement="top-start">
                            <IconButton
                              component={Link}
                              to={`/administration/test-type/view/${row.id}`}
                              aria-label="save"
                              className={classes.margin}
                              size="small"
                            >
